...
03と同じUNION Injectionだが、今回は`UNION`という入力がブロックされる。
この問題のテーマは、SQLキーワードの大文字小文字だけを変えてフィルタを通ること。

注入のやり方自体は03の使い回しでよい。新しく越えるべき壁は「ブラックリスト型フィルタは
文字列の表記ゆれに弱い」という1点だけ。ここで掴む考え方が05(値を16進数で表す回避)にも
そのまま繋がる。
