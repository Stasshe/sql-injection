...
| 02 | http://localhost:3002 | ORで全件表示 | 入門 |
| 03 | http://localhost:3003 | UNION-based抽出 | 基礎 |
...
02, 04, 05の違い(いずれもフィルタ・条件の回避対象が異なる):

- 02は、数値条件へ`OR 1=1`を足す基本(フィルタなし)。
- 04は、SQLキーワードの大文字小文字を変えてブラックリストを回避。
- 05は、文字列をUTF-8の16進数へ変えて平文フィルタを回避。

04は03(UNION)と同じpayloadの構造をほぼ再利用しつつ、ブラックリスト型フィルタという
新しい壁を越える回だと考えると位置づけが分かりやすい。
